...
class TestBenchmarkCrossDevice:
    ...
    @staticmethod
    def train(sceduling: Iterable[str], epoch: int=10, batch: int=4096):
        ...
        w0, apply0 = mlp(input_shape=(28 * 28,), nodes_per_layer=[1024] * 8)
        w0, opt0, forward0, backward0 = async_host(device=gpus[0], weights=w0, apply_fn=apply0, optimizer=optimizer)

        w1, apply1 = mlp(input_shape=(1024,), nodes_per_layer=[1024] * 8 + [10])
    ...
    def test_simple_model_parallelism(self):
        ...
        scheduling = cycle(["forward", "backward", "forward", "backward"])
    ...
    def test_gpipe_scheduling(self):
        ...
        """
        scheduling = cycle(["forward", "forward", "backward", "backward"])
        self.train(scheduling)
